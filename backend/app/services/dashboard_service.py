...
class DashboardService:
    ...
    def get_overview(self) -> Dict[str, Any]:
        ...
        active_alerts = self.alert_repo.count_firing()
        ...
        top_impacted = self._get_top_impacted()
        active_incident_list = self._get_active_incidents()
        env_health = self._get_environment_health()
        connector_health = self._get_connector_health()
        ai_highlights = self._get_ai_highlights()
    ...
    def get_top_impacted(self, limit: int = 6) -> List[Dict[str, Any]]:
        return self._get_top_impacted(limit)
    ...
    def _get_top_impacted(self, limit: int = 6) -> List[Dict[str, Any]]:
        impacted = self.app_repo.get_impacted()
        result = []
    ...
    def _get_active_incidents(self, limit: int = 5) -> List[Dict[str, Any]]:
        incidents = self.incident_repo.get_active()
        return [
            {
                "id": inc.id,
                "title": inc.title,
    ...
    def _get_ai_highlights(self, limit: int = 3) -> List[Dict[str, Any]]:
        insights = self.insight_repo.get_recent(limit)
        return [
            {
                "id": i.id,
                "title": i.title,
